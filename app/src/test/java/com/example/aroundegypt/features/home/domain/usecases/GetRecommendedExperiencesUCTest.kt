...
class GetRecommendedExperiencesUCTest {

    @Mock
    private lateinit var repository: IExperiencesRepository

    private lateinit var getRecommendedExperiencesUC: GetRecommendedExperiencesUC

    @Before
    fun setUp() {
        MockitoAnnotations.openMocks(this)
        getRecommendedExperiencesUC = GetRecommendedExperiencesUC(repository)
    }
    ...
    fun `invoke should emit Loading followed by Success when repository returns data`() = runTest {
        ...
        whenever(repository.getRecommendedExperiences()).thenReturn(recommendedExperiences)

        // Act & Assert
        getRecommendedExperiencesUC().test {
            // Loading state
            ...
            assertEquals(recommendedExperiences, (success as Resource.Success).data)
    ...
    fun `invoke should emit Loading followed by Failure with UnknownHostException`() = runTest {
        ...
        whenever(repository.getRecommendedExperiences()).thenAnswer { throw (unknownHostException) }
    ...
    fun `invoke should emit Loading followed by Failure with SocketTimeoutException`() = runTest {
        ...
        whenever(repository.getRecommendedExperiences()).thenAnswer { throw (socketTimeoutException) }
    ...
    fun `invoke should emit Loading followed by Failure with UnknownException`() = runTest {
        ...
        whenever(repository.getRecommendedExperiences()).thenThrow(unknownException)
        ...
        getRecommendedExperiencesUC().test {
            ...
            assertEquals(
                "Unknown error in GetRecommendedExperiencesUC: $unknownException",
