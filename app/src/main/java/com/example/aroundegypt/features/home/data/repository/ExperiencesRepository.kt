...
class ExperiencesRepository(
    ...
    override suspend fun getMostRecentExperiences(): List<Experience> {
        if (connectivityChecker.isInternetAvailable()) {
    ...
    override suspend fun searchExperiences(query: String): List<Experience> {
        if (connectivityChecker.isInternetAvailable()) {
    ...
    override suspend fun getLikedExperiences(): List<Experience> {
        return localDS.getLikedExperiences().map { ExperienceMapper.entityToDomain(it) }
    }

    override suspend fun saveLikedExperiences(likedExperiences: List<Experience>) {
