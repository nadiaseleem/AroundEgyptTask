...
class LikeExperienceUC(private val repository: IExperiencesRepository) {
    operator fun invoke(query: String) = flow {
        emit(Resource.Loading())
        val updatedLikeNo = repository.likeExperience(query)
        emit(Resource.Success(updatedLikeNo))
    }.catch { throwable ->
        val exception =
            when (throwable) {
                ...
                else -> AroundEgyptException.UnknownException("Unknown error in LikeExperienceUC: $throwable")
            }
